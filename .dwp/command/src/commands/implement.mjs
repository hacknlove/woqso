...
const promptPath = path.join(commandDir, 'prompts', 'implement.md')
...
export async function main({ env = process.env, runtime = createRuntime() } = {}) {
  ...
  try {
    ...
    const runTitle = `${commitHash}-implement`
    ...
    const implementerSessionId = await findSessionId(runtime, { repoRoot, title: runTitle })
    requireValue(implementerSessionId, 'Unable to determine opencode session id')
    ...
    const nextSubjectMap = {
      'review-implementation': `implement: create ${ticketName} implementation`,
      'revisit-plan': `implement: revisit ${ticketName} plan`,
      'call-human': `implement: needs human input for ${ticketName}`,
    }
    ...
    await failWithError({
      ...
      failureSummary: 'Command failed while creating the initial implementation.',
      cause: error.message,
      stageOnError: 'repo',
    })
    process.exitCode = 1

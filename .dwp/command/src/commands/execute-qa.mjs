...
const promptPath = path.join(commandDir, 'prompts', 'execute-qa.md')
...
export async function main({ env = process.env, runtime = createRuntime() } = {}) {
  ...
  try {
    ...
    const plannerSessionId = requireValue(getTrailer('DWP_PLANNER_SESSION_ID', env), 'Missing required session-id trailer')
    ...
    const implementerSessionId = requireValue(getTrailer('DWP_IMPLEMENTER_SESSION_ID', env), 'Missing required session-id trailer')
    ...
    const qaPlannerSessionId = requireValue(getTrailer('DWP_QA_PLANNER_SESSION_ID', env), 'Missing required session-id trailer')
    ...
    const runTitle = `${commitHash}-execute-qa`
    ...
    const decision = parseDecision(outputBody, ['deploy', 'iterate-qa-plan', 'iterate-implementation', 'call-human'])

    const nextSubjectMap = {
      deploy: `execute-qa: approve ${ticketName} for deploy`,
      'iterate-qa-plan': `execute-qa: request ${ticketName} qa plan changes`,
      'iterate-implementation': `execute-qa: request ${ticketName} implementation changes`,
      'call-human': `execute-qa: needs human input for ${ticketName}`,
    }
    ...
    await failWithError({
      ...
      repoRoot,
      ticketPath,
      commandName: 'execute-qa',
      failureSummary: 'Command failed while executing QA.',
      cause: error.message,
      stageOnError: 'repo',
    })
    process.exitCode = 1

...
const promptPath = path.join(commandDir, 'prompts', 'review-plan.md')
...
export async function main({ env = process.env, runtime = createRuntime() } = {}) {
  ...
  try {
    const body = requireValue(getBody(env), 'Missing latest iteration notes for reviewer in commit body')
    ...
    const runTitle = `${commitHash}-review-plan`
    ...
    const decision = parseDecision(outputBody, ['implement', 'iterate-plan', 'call-human'])
    const nextSubjectMap = {
      implement: `review-plan: approve ${ticketName} plan`,
      'iterate-plan': `review-plan: request ${ticketName} plan changes`,
      'call-human': `review-plan: needs human input for ${ticketName}`,
    }
    ...
    await failWithError({
      runtime,
      repoRoot,
      commandName: 'review-plan',
      failureSummary: 'Command failed while reviewing the plan.',
      cause: error.message,
    })
    process.exitCode = 1
  }
